# This Makefile is for the File::Path::Localize extension to perl.
#
...
#	NAME => q[File::Path::Localize]
#	VERSION_FROM => q[lib/File/Path/Localize.pm]
#	clean => { FILES=>q[*.bak *~] }
#	depend => { dist=>q[File-Path-Localize.spec], Makefile=>q[$(VERSION_FROM)], 0spec=>q[$(VERSION_FROM)] }
#	dist => { COMPRESS=>q[gzip --force --best] }
#	realclean => { FILES=>q[File-Path-Localize-*.tar.gz File-Path-Localize.spec] }
...
DISTNAME = File-Path-Localize
NAME_SYM = File_Path_Localize
VERSION = 1.0.0
VERSION_SYM = 1_0_0
XS_VERSION = 1.0.0
INST_BIN = blib/bin
...
HTMLSCRIPTPODS = 
MAN1PODS = 
MAN3PODS = lib/File/Path/Localize.pm
HTMLEXT = html
...
# Where to put things:
INST_LIBDIR      = $(INST_LIB)/File/Path
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/File/Path
...
TO_INST_PM = lib/File/Path/Localize.pm

PM_TO_BLIB = lib/File/Path/Localize.pm \
	$(INST_LIB)/File/Path/Localize.pm
...
manifypods : pure_all lib/File/Path/Localize.pm
	@$(POD2MAN) \
	lib/File/Path/Localize.pm \
	$(INST_MAN3DIR)/File::Path::Localize.$(MAN3EXT)

# --- MakeMaker processPL section:

all :: File-Path-Localize.spec
	@$(NOOP)

File-Path-Localize.spec :: File-Path-Localize.spec.PL
	$(PERL) -I$(INST_ARCHLIB) -I$(INST_LIB) -I$(PERL_ARCHLIB) -I$(PERL_LIB) File-Path-Localize.spec.PL File-Path-Localize.spec
...
clean ::
	-rm -rf *.bak *~ ./blib $(MAKE_APERL_FILE) $(INST_ARCHAUTODIR)/extralibs.all perlmain.c mon.out core core.*perl.*.? *perl.core so_locations pm_to_blib *$(OBJ_EXT) *$(LIB_EXT) perl.exe $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def $(BASEEXT).exp
...
realclean purge ::  clean
	...
	rm -f  $(INST_LIB)/File/Path/Localize.pm
	rm -rf Makefile Makefile.old File-Path-Localize-*.tar.gz File-Path-Localize.spec
...
ppd:
	@$(PERL) -e "print qq{<SOFTPKG NAME=\"File-Path-Localize\" VERSION=\"1,0,0,0\">\n}. qq{\t<TITLE>File-Path-Localize</TITLE>\n}. qq{\t<ABSTRACT></ABSTRACT>\n}. qq{\t<AUTHOR></AUTHOR>\n}. qq{\t<IMPLEMENTATION>\n}. qq{\t\t<OS NAME=\"$(OSNAME)\" />\n}. qq{\t\t<ARCHITECTURE NAME=\"i386-linux\" />\n}. qq{\t\t<CODEBASE HREF=\"\" />\n}. qq{\t</IMPLEMENTATION>\n}. qq{</SOFTPKG>\n}" > File-Path-Localize.ppd
